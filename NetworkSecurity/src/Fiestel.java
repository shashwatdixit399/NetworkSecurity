...
class Fiestel { 
	private static class DES { 
		...
		{ 
			String output = ""; 
			input = convtobin(input); 
			for (int i = 0; i < sequence.length; i++) 
				output += input.charAt(sequence[i] - 1); 
			output = convtohex(output); 
			return output; 
		} 
	...
	{ 
		String text = "123456CBDE123456"; 
		String key = "AABB09182736CCDD"; 

		DES obj = new DES(); 
		text = obj.encrypt(text, key); 
		System.out.println( 
			"\nCipher Text: " + text.toUpperCase() + "\n"); 
		text = obj.decrypt(text, key); 
		System.out.println( 
			"\nPlain Text: "
			+ text.toUpperCase()); 
		System;
